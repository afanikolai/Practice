...
namespace Practice_11
{
    ...
    //  программа выдает переработанный текст. Файл с шифром представляет собой текстовый документ, в строках которого последовательно 
    //  расположены n пар вида "буква исходного алфавита - ее шифр". Пользователь сам должен убедиться, что система шифрования является 
    //  однозначно декодируемой, что коды никаких двух букв не повторяются, что ни один документ не содержит кириллических символов и 
    //  что в шифре представлены все символы, которые используются в исходном тексте. В файле с шифром каждый исходный символ и каждый 
    //  код должен состоять из одного символа. 
                    ...
                    case 1: // Выбрать файл с шифром.
                        ...
                        try
                        {
                            ...
                            while (tmp != null)   // Пока не последняя строка. 
                            {
                                ...
                                if (!keys.ContainsKey(tmpkey[0][0]) && (!keys.ContainsValue(tmpkey[1][0])))
                                {
                                ...
                                {
                                    throw new Exception("Шифр и алфавит не должны содержать повторяющихся символов.");
                                }
                            }
